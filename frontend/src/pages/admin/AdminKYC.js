...
function KycDocViewer({ userId, zoom }) {
  ...
  const imageUrl = `${API_URL}/api/admin/kyc/document/${userId}/${activeDoc === 'id' ? 'id' : 'selfie'}`;

  return (
    <div className="admin-kyc-viewer-shell">
      <div className="admin-kyc-viewer-tabs">
        <button
          className={`admin-filter-chip ${activeDoc === 'id' ? 'active' : ''}`}
      ...
      <div className="admin-kyc-viewer-stage">
        <img
          key={imageUrl}
          src={imageUrl}
          alt={activeDoc === 'id' ? 'ID document' : 'Selfie with ID'}
          style={{
        ...
        />
        <div className="admin-kyc-viewer-empty" style={{ display: 'none' }}>
          <div style={{ fontSize: '14px' }}>
            {activeDoc === 'id' ? 'ID document is not available.' : 'Selfie with ID is not available.'}
          </div>
        </div>
      </div>
    </div>
...
export default function AdminKYC() {
  ...
  const fetchKycQueue = async ({ silent = false } = {}) => {
    ...
    try {
      ...
      const filteredRows = (next.rows || []).filter((row) => ['pending', 'approved', 'rejected'].includes(String(row.kyc_status || '').toLowerCase()));
  ...
  const rows = useMemo(() => (
    (listData.rows || []).map((row) => {
      ...
      return {
        ...
        missing_files: Boolean(quality && (!quality.id_file_exists || !quality.selfie_file_exists))
      };
    })
          ...
          {selectedUser ? (
            <>
              <div className="admin-kyc-review-header">
                <div>
                  ...
                  <div style={{ color: 'var(--admin-text-muted)', fontSize: '13px', marginTop: '4px' }}>
                    {selectedUser.email} - {selectedUser.country || 'Unknown Country'}
                  </div>
                </div>
              ...
              <div className="admin-entity-info-grid" style={{ marginBottom: '16px' }}>
                <div><span>Submitted</span><strong>{formatDate(selectedUser.created_at)}</strong></div>
              ...
              <div className="admin-card" style={{ margin: 0 }}>
                {String(selectedUser.kyc_status || '').toLowerCase() === 'rejected' ? (
                  ...
                  <KycDocViewer userId={selectedUser.id} zoom={zoom} />
                )}
              </div>
