...
export default function GetChallenge({ onCreateAccount, kycStatus, setActivePage }) {
  ...
  function getClaimLimitMessage(data) {
    const limit = data?.monthly_claim_limit
    if (!limit) return data?.error || 'Free account already claimed. Try again next month.'
    const resetText = limit.resets_at
      ? new Date(limit.resets_at).toLocaleDateString(undefined, { month: 'long', day: 'numeric' })
      : 'next month'
    return `Free account already claimed. Try again ${resetText}.`
  }
  ...
  const loadAll = useCallback(async () => {
    ...
    try {
      const [sizesRes, rulesRes, limitRes] = await Promise.all([
        ...
        api.get('/api/accounts/my-purchase-limit').catch(() => ({ data: null })),
      ])
      setSizes(sizesRes.data || [])
      setRules(rulesRes.data || {})
      setPurchaseLimit(limitRes.data || null)
    } finally {
      setLoading(false)
    }
  }, [])
  ...
  async function handleConfirm() {
    ...
    setError('')
    try {
      const orderRes = await api.post('/api/accounts/orders', { account_size: confirmSize }, { skipAuthRedirect: true })
      ...
      const message = data?.monthly_claim_limit
        ? getClaimLimitMessage(data)
  ...
  const checkoutMode = String(rules.challenge_checkout_mode || tenant?.settings?.challenge_checkout_mode || (requiresPayment ? 'paid' : 'free')).toLowerCase()
  const isFreeCheckout = !(requiresPayment || checkoutMode === 'paid')
  ...
  const claimLimitReached = isFreeCheckout && !!purchaseLimit?.limit_reached
  const claimLimitReset = purchaseLimit?.resets_at
    ? new Date(purchaseLimit.resets_at).toLocaleDateString(undefined, { month: 'short', day: 'numeric' })
    : 'next month'
    ...
    <div style={{ maxWidth: '1100px' }}>
      ...
      {claimLimitReached && (
        <div style={{
          ...
          <span style={{ display: 'inline-flex' }}>
            {renderIcon('calendar', { size: 28, color: 'var(--accent)' })}
          </span>
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: '14px', fontWeight: 800, color: 'var(--accent)', marginBottom: '4px' }}>
              Monthly Free Claim Used
            </div>
            <div style={{ fontSize: '13px', color: 'var(--text-muted)' }}>
              You have claimed {purchaseLimit.used} of {purchaseLimit.max} free challenge account{Number(purchaseLimit.max) === 1 ? '' : 's'} this month.
              New free claims reopen on {claimLimitReset}.
            </div>
          </div>
        </div>
      )}
      ...
      {loading ? (
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: '18px', marginBottom: '40px' }}>
          {sizes.map(({ size, locked, remaining, quota, reason }) => {
            const isLocked   = locked || kycBlocked || claimLimitReached
              ...
              >
                ...
                <div style={{ position: 'absolute', top: '14px', right: '14px' }}>
                  {locked ? (
                    ...
                    <Pill color="var(--accent)">MONTHLY LIMIT</Pill>
                  ) : almostFull ? (
