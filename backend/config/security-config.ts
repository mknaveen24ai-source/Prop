...
interface SecurityMonitor {
  ...
  detectSuspiciousActivity: RequestHandler
  checkAttackPatterns: RequestHandler
}
...
const securityMonitor: SecurityMonitor = {
  // Log security events
  logSecurityEvent: (event: string, details: unknown): void => {
  ...
  detectSuspiciousActivity: (req, res, next) => {
    const patterns = {
      rapidRequests: (req.requestCount ?? 0) > 100,
      adminAccess: req.path.includes('/admin') && !req.user?.isAdmin,
      blockedCountry: typeof req.country === 'string' && securityConfig.blockedCountries.includes(req.country),
      unusualUserAgent: (req.get('User-Agent')?.length ?? Number.POSITIVE_INFINITY) < 10,
    };
  ...
  checkAttackPatterns: (req, res, next) => {
    ...
    const requestString = String(JSON.stringify(requestBody)) + req.path + req.get('User-Agent');
