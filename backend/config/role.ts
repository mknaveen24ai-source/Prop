...
const VALID_ROLES = ['all', 'engine', 'gateway', 'api'] as const
type ProcessRole = typeof VALID_ROLES[number]

function isProcessRole(value: string): value is ProcessRole {
  return value === 'all' || value === 'engine' || value === 'gateway' || value === 'api'
}
 ...
 */
function publishesRealtimeOverRedis(): boolean {
  return ROLE === 'engine'
}
