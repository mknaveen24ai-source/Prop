...
function isUnsafeHexKey(value: string): boolean {
  return !/^[0-9a-f]{32,}$/i.test(value)
}

function isUnsafeDatabaseUrl(value: string): boolean {
...
function isUnsafeTrustProxy(value: string): boolean {
...
function isUnsafeSmtpFrom(smtpFrom: string, smtpUser: string | undefined): boolean {
  if (smtpFrom === smtpUser) return true
  return ESP_TECHNICAL_DOMAIN_PATTERN.test(smtpFrom)
}

function getUnsafeProductionEnvVars(): string[] {
  const unsafe = []
