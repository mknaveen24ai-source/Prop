 ...
 */
function getRequestIp (req, fallback = UNKNOWN) {
...
function getIpSubnet (rawIp) {
  ...
  if (isIPv6(value)) {
    ...
    if (/^fe[89ab]/.test(groups[0])) return null  // fe80::/10 link-local
    if (/^f[cd]/.test(groups[0])) return null     // fc00::/7 unique-local
...
module.exports = { getRequestIp, getIpSubnet, UNKNOWN }
