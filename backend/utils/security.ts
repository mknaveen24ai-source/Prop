...
import type * as RateLimitRedisApi from 'rate-limit-redis'
import type { RedisReply } from 'rate-limit-redis'
import type * as TokenCacheApi from './tokenCache'
...
type LimiterOptions = Partial<RateLimitOptions>
...
function makeSharedStore(prefix: string): Store {
  const { getRedisClient } = require('./tokenCache') as typeof TokenCacheApi
  const { RedisStore } = require('rate-limit-redis') as typeof RateLimitRedisApi
  ...
  const redisStore = new RedisStore({
    ...
    sendCommand: async (...args: string[]): Promise<RedisReply> => {
      ...
      const reply: unknown = await client.sendCommand(args)
      if (!isRedisReply(reply)) throw new Error('Redis returned an invalid rate-limit reply')
      return reply
    }
  });

  let initOptions: RateLimitOptions | null = null;
  ...
  function warnOnce(error: unknown): void {
    ...
    warnedNoRedis = true;
    logger.warn(
      '[security] Rate limiting fell back to in-process counters: ' + errorMessage(error) + '. ' +
 ...
 */
function createLimiter(name: string, options: LimiterOptions = {}): RateLimitRequestHandler {
  return rateLimit({
    ...
    handler: (req, res, _next, opts) => {
      ...
      const configuredMessage: unknown = opts.message
      const body = typeof configuredMessage === 'string'
...
const tradingLimiter = createLimiter('trading', {
  ...
  legacyHeaders: false,
  keyGenerator: (req) => {
    return req.user?.userId || ipKeyGenerator(normalizedIp(req.ip));
  }
});
...
const apiLimiter = createLimiter('api', {
  ...
  keyGenerator: (req) => ipKeyGenerator(normalizedIp(req.ip))
});
...
const kycLimiter = createLimiter('kyc', {
  ...
  standardHeaders: true,
  legacyHeaders: false,
  keyGenerator: (req) => req.user?.userId || ipKeyGenerator(normalizedIp(req.ip))
});
...
const fileUploadSecurity: RequestHandler = (req, res, next): void => {
  ...
  if (req.file && req.file.mimetype.startsWith('image/')) {
    ...
    if (!allowedTypes.includes(req.file.mimetype)) {
      res.status(400).json({ error: 'Invalid image type. Only JPEG, PNG, GIF, and WebP are allowed.' });
      return
    }
  }
...
const abusiveIPs = new Map<string, AbuseEntry>()
...
const abuseDetector: RequestHandler = (req, res, next): void => {
  ...
  if (abusiveIPs.size > MAX_ABUSIVE_IPS) {
    ...
    if (abusiveIPs.size > MAX_ABUSIVE_IPS) {
      ...
      for (let i = 0; i < toRemove; i++) {
        ...
        if (entry) abusiveIPs.delete(entry[0])
      }
...
const requestSizeLimiter: RequestHandler = (req, res, next): void => {
