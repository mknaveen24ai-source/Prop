...
const { getInstrumentConfig, isFxConversionEnabled } = require('../instruments')
...
function readPeggedRate(source, currency) {
...
function getUsdRate(currency) {
  ...
  if (source.pegged) return readPeggedRate(source, normalized)
 ...
 */
function getUsdRateForInstrument(instrument) {
...
function isUsdQuoted(instrument) {
  const config = getInstrumentConfig(instrument)
...
function getRateSnapshot() {
  ...
  for (const currency of Object.keys(RATE_SOURCES)) {
    try {
      ...
      snapshot[currency] = { rate: null, available: false, reason: error.message }
    }
  }
  return snapshot
}
