...
// FIX (F-02): profit share resolves through constants.js so the seeded default
// below, the payout calculation, the account rules payload and the analytics
// forecast all read the same number. See the note there for the full history.
interface ConstantsApi {
  PROFIT_SHARE_FALLBACK_PCT: number
  resolveProfitSharePct: (rawValue: unknown) => number | null
}
...
const { PROFIT_SHARE_FALLBACK_PCT, resolveProfitSharePct } = require('../constants') as ConstantsApi
const settingMapSchema = z.record(z.string(), z.unknown())

const DEFAULT_TENANT_SETTINGS: Readonly<Record<string, string>> = {
  ...
  profit_share_pct: String(PROFIT_SHARE_FALLBACK_PCT),
...
let challengeCommerceInfrastructurePromise: Promise<void> | null = null
...
// SaaS billing layer.
async function ensureTenantSettingsInfrastructure(): Promise<void> {
  ...
  challengeCommerceInfrastructurePromise = (async () => {
    ...
    logger.error('[settings] Failed to ensure challenge commerce infrastructure:', {
...
async function getTenantSettingsMap(keys: readonly string[] = []): Promise<Record<string, string>> {
  const useKeyFilter = Array.isArray(keys) && keys.length > 0
  const result = useKeyFilter
    ? await pool.query<SettingRow>(`SELECT key, value FROM platform_settings WHERE key = ANY($1::text[])`, [keys])
    : await pool.query<SettingRow>(`SELECT key, value FROM platform_settings`)

  const settings: Record<string, string> = { ...DEFAULT_TENANT_SETTINGS }
  for (const row of result.rows) settings[row.key] = row.value
  return settings
}

async function getTenantSettingValue(key: string, fallback: string | null = null): Promise<string | null> {
...
async function ensureTenantSettingDefaults(
  clientOrPool: unknown,
  defaults: Readonly<Record<string, unknown>> = DEFAULT_TENANT_SETTINGS
): Promise<void> {
  const db = isQueryable(clientOrPool) ? clientOrPool : pool
...
function parseSpreadMarkupMap(rawValue: unknown): Record<string, unknown> {
  if (!rawValue) return {}
  if (typeof rawValue === 'object') {
    const parsedObject = settingMapSchema.safeParse(rawValue)
    return parsedObject.success ? parsedObject.data : {}
  }
  try {
    const parsed: unknown = JSON.parse(String(rawValue))
    const result = settingMapSchema.safeParse(parsed)
...
function getInstrumentSpreadMarkup(settings: unknown, instrument: string): number {
  const settingsMap = settingMapSchema.safeParse(settings)
  const spreadMap = parseSpreadMarkupMap(
    settingsMap.success ? settingsMap.data.spread_markup_points_json : undefined
  )
...
// funded, competition.
function normalizeAccountTypeTier(accountType: unknown): 'challenge' | 'funded' | 'competition' {
...
function resolveTieredInstrumentSetting(
  ...
  const rawTierMap = map[tier]
  const tierMap = settingMapSchema.safeParse(rawTierMap)
  const tierSpecific = Number(tierMap.success ? tierMap.data[instrument] : undefined)
  if (Number.isFinite(tierSpecific)) return tierSpecific
  const tierWildcard = Number(tierMap.success ? tierMap.data['*'] : undefined)
  if (Number.isFinite(tierWildcard)) return tierWildcard

  const rawGlobalTierMap = map['*']
  const globalTierMap = settingMapSchema.safeParse(rawGlobalTierMap)
  const globalSpecific = Number(globalTierMap.success ? globalTierMap.data[instrument] : undefined)
  if (Number.isFinite(globalSpecific)) return globalSpecific
  const globalWildcard = Number(globalTierMap.success ? globalTierMap.data['*'] : undefined)
