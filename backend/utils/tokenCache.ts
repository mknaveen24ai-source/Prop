...
async function initializeRedis(): Promise<RedisClient | null> {
  ...
  redisInitPromise = (async () => {
    let candidate: RedisClient | null = null
    try {
      ...
      logger.error('Failed to initialize Redis:', { error: errorMessage(err) })
      if (candidate) {
        try {
          candidate.destroy()
        } catch {}
 ...
 */
async function getCachedTokenData(userId: string): Promise<CachedTokenData | null> {
  ...
  try {
    ...
    logger.warn('Redis cache get failed, falling back to DB:', { error: errorMessage(err) })
    return null
  }
}
...
async function cacheTokenData(
  ...
  try {
    ...
    logger.warn('Failed to cache token data:', { error: errorMessage(err) })
  }
}
 ...
 */
async function invalidateTokenCache(userId: string): Promise<boolean> {
 ...
 */
async function invalidateAllUserTokens(userId: string): Promise<boolean> {
...
async function closeRedis(): Promise<void> {
  if (redisClient) {
    try {
      ...
      logger.error('Error closing Redis:', { error: errorMessage(err) })
    }
  }
}
