 ...
 * Never throws.
 */
function parseDeviceSignature (headerValue) {
  try {
    ...
    const components = sanitizeComponents(payload && payload.components)
    if (!components) return null

    const hash = computeFingerprint(components)
    if (!hash) return null

    return { hash, components, version: Number(payload.v) || 1 }
  } catch {
    return null
  }
}
 ...
 */
function deviceSignatureMiddleware (req, res, next) {
