...
const DIRECTION_BUY = 1 as const
const DIRECTION_SELL = -1 as const
type DirectionSign = typeof DIRECTION_BUY | typeof DIRECTION_SELL
...
function directionSign(direction: string): DirectionSign {
  return direction === 'buy' ? DIRECTION_BUY : DIRECTION_SELL
}

function contractSizeFor(instrument: string): number {
 ...
 */
function closePriceFor(sign: DirectionSign, price: BidAskPrice): number {
...
function isSLTriggered(sign: DirectionSign, stopLoss: number | null | undefined, currentPrice: number): boolean {
...
function isTPTriggered(sign: DirectionSign, takeProfit: number | null | undefined, currentPrice: number): boolean {
