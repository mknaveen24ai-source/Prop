...
let tenantFeedInfrastructurePromise = null
const lastHeartbeatPersistedAt = new Map()
const lastHeartbeatPersistedStatus = new Map()
const lastSourceHourlyPersistedAt = new Map()
const heartbeatPersistPromises = new Map()
const sourceHourlyPersistPromises = new Map()
...
function normalizeFeedMode(value, fallback = 'shared') {
  const normalized = String(value || fallback).trim().toLowerCase()
  return ['shared', 'dedicated'].includes(normalized) ? normalized : fallback
}

async function listActivePriceFeedSources() {
...
async function listManagedPriceFeedSources() {
  await ensureTenantFeedInfrastructure()
  const result = await pool.query(
    `SELECT *
       FROM price_feed_sources
...
async function getPriceFeedSource(sourceKey) {
  await ensureTenantFeedInfrastructure()
...
async function markPriceFeedSourceHeartbeat(sourceKey, options = {}) {
  await ensureTenantFeedInfrastructure()
  const normalizedSourceKey = String(sourceKey || '').trim().toLowerCase()
...
async function markPriceFeedSourceError(sourceKey, errorMessage) {
  await ensureTenantFeedInfrastructure()
  const normalizedSourceKey = String(sourceKey || '').trim().toLowerCase()
  await pool.query(
    `UPDATE price_feed_sources
        SET status = 'error',
            ...
            updated_at = NOW()
      WHERE source_key = $1`,
    [normalizedSourceKey, String(errorMessage || 'Unknown price feed error')]
  )
  lastHeartbeatPersistedAt.set(normalizedSourceKey, Date.now())
  lastHeartbeatPersistedStatus.set(normalizedSourceKey, 'error')
}

async function upsertSourcePrices(sourceKey, priceRows = []) {
  await ensureTenantFeedInfrastructure()
...
async function upsertSourceHourlyTicks(sourceKey, priceRows = []) {
  if (!Array.isArray(priceRows) || priceRows.length === 0) return
...
async function appendSourcePriceHistory(sourceKey, priceRows = []) {
  await ensureTenantFeedInfrastructure()
  if (!Array.isArray(priceRows) || priceRows.length === 0) return
...
async function getCurrentPricesForSource(sourceKey) {
  await ensureTenantFeedInfrastructure()
...
async function getCurrentPriceForSource(sourceKey, instrument) {
...
async function upsertTenantPriceFeedConfig(clientOrPool, config = {}) {
  ...
  const spreadMarkupPointsJson = hasExplicitSpreadMarkup
    ? config.spread_markup_points_json
    : existingConfig?.spread_markup_points_json ?? null
  const metadataJson = config.metadata_json !== undefined
